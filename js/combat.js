...
stop=function(){
  ...
  if(rt==='combat_repeatable'){
    ...
    if(rid==='p4_etech'){var tech=item._tech;if(!tech){showToast('无效技法');return}c.enemyStamina=Math.max(0,c.enemyStamina-tech.st);if(tech.ce>0)c.enemyCe-=tech.ce;c.enemyWin+=tech.win;c.log.push('[T'+c.round+'] 👤 '+item.l+': -'+tech.st+'体 +'+tech.win+'胜');var eBF=false;var eIsAtk=tech.tier&&tech.tier.indexOf('atk')>=0;if(eIsAtk&&Math.random()*100<v3EnemyBfRate()){eBF=true;c.enemyCe+=5;c.enemyWin+=Math.max(1,Math.floor(v3EnemyBfRate()*2));c.log.push('[T'+c.round+'] 👤⚡黑闪!');showToast('👤⚡黑闪!')}var eAvail=v3BuildCombatItems(true);if(c.enemyStamina<=0||!eAvail||eAvail.length===0){c.phase='clash';var doneAlready=state.results.filter(function(rr){return rr.roundId==='p4_etech'});if(doneAlready.length===0)state.results.push({roundId:'p4_etech',rname:'🗡 敌出招',prop:'',label:'完毕',desc:'',c:'#d84',_item:{tags:[],dim:{},dimMod:{}}});showToast('敌体力耗尽→对拼');var eNxt=activeRounds().findIndex(function(rx){return rx.id==='p4_clash'});if(eNxt>=0)goRound(eNxt);else updateCombatUI();saveState();return}document.getElementById('btnSpin').style.display='none';document.getElementById('btnNext').style.display='block';document.getElementById('btnNext').textContent='→ 下一招';updateCombatUI();saveState();return}
