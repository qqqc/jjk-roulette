...
function roundStamina(){
  ...
  if(c.enemyBurnout){var e=ENEMY_TEMPLATES[c.enemyId];if(e&&e.dim){var wi=e.dim['意志']?dimVal(e.dim['意志']):3,ci=e.dim['咒力操纵']?dimVal(e.dim['咒力操纵']):3;var rRate=10+(wi-3)*2+(ci-3);if(Math.random()*100<rRate){c.enemyBurnout=false;c.enemyCe=Math.max(0,c.enemyCe-Math.floor(v3EnemyCeMax()*0.3));c.enemyShield=Math.floor(c.enemyCe*0.5);showToast('敌熔断修复!')}}}
