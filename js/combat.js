...
function v3EnemyDrawCe(){var e=ENEMY_TEMPLATES[state.combat.enemyId];if(!e||!e.dim)return 0;var v=e.dim['咒力总量'];if(v==='E-')return 0;var mx=_CE[Math.max(0,Math.min(9,dimVal(v)))],willI=e.dim['意志']?dimVal(e.dim['意志']):3,lo=_CE_DL[Math.max(0,Math.min(9,willI))];return Math.floor(mx*lo/100)}
...
function v3BuildCombatItems(forEnemy){
  ...
  if(!forEnemy){
    ...
    return techs.map(function(t){var st=Math.max(1,Math.floor((t.st||8)*1)),ce=Math.floor((t.ce||0)*1);var win=(t.win||20)+v3EnemyWinBonus();return{l:t.name,w:8,c:t.c||'#d84',d:t.eff||'',_tech:{st:st,ce:ce,win:win,id:t.id,tier:t.tier||'atk'}}}).filter(function(t){if(c.enemyCe===0&&t._tech.ce>0)return false;if(c.enemyBlocked&&t._tech.tier&&t._tech.tier.indexOf('atk_ce')>=0)return false;if(t._tech.st>c.enemyStamina)return false;if(t._tech.ce>c.enemyCe)return false;return true})
  }
}
...
function _v3HandleEnemyPrepStop(r){
  state.spinning=false;var norm=(-wheel.angle)%(Math.PI*2);if(norm<0)norm+=Math.PI*2;var cum=0,idx=0;
  for(var i=0;i<wheel.sectors.length;i++){cum+=wheel.sectors[i].arc;if(norm<cum){idx=i;break}}
  var ceVal=parseInt(wheel.sectors[idx].l)||0;state.combat.enemyCe=ceVal;state.combat.enemyShield=Math.floor(ceVal*0.5);
  updateCombatUI();
...
function updateCombatUI(){
  ...
  if(c.yourDomainActive){var dEff=c.domainEffect||'增幅术式';var dType=c.domainType||'封闭式';document.getElementById('cvDomainBadge').textContent='🌐 '+dType+'·'+dEff+' 剩'+c.domainRemaining+'回合';document.getElementById('cvDomainBadge').title=dType+'领域: '+dEff+'效果'}
  // 偏袒显示
