...
stop=function(){
  ...
  if(rt==='combat_repeatable'){
    if(rid==='p4_ptech'){var tech=item._tech;if(!tech){showToast('无效技法');return}
      ...
      var rp=document.getElementById('resultPanel');rp.style.display='block';var bfExtra=isBF?' ⚡黑闪! ':'';
    ...
    if(rid==='p4_etech'){var tech=item._tech;if(!tech){showToast('无效技法');return}c.enemyStamina=Math.max(0,c.enemyStamina-tech.st);if(tech.ce>0)c.enemyCe-=tech.ce;c.enemyWin+=tech.win;c.log.push('[T'+c.round+'] 👤 '+item.l+': -'+tech.st+'体 +'+tech.win+'胜');var eBF=false;var eIsAtk=tech.tier&&tech.tier.indexOf('atk')>=0;if(eIsAtk&&Math.random()*100<v3EnemyBfRate()){eBF=true;if(c.enemyCe>0)c.enemyCe+=5;c.enemyWin+=Math.max(1,Math.floor(v3EnemyBfRate()*2));c.log.push('[T'+c.round+'] 👤⚡黑闪!');showToast('👤⚡黑闪!')}var eAvail=v3BuildCombatItems(true);if(c.enemyStamina<=0||!eAvail||eAvail.length===0){c.phase='clash';var doneAlready=state.results.filter(function(rr){return rr.roundId==='p4_etech'});if(doneAlready.length===0)state.results.push({roundId:'p4_etech',rname:'🗡 敌出招',prop:'',label:'完毕',desc:'',c:'#d84',_item:{tags:[],dim:{},dimMod:{}}});showToast('敌体力耗尽→对拼');var eNxt=activeRounds().findIndex(function(rx){return rx.id==='p4_clash'});if(eNxt>=0)goRound(eNxt);else updateCombatUI();saveState();return}document.getElementById('btnSpin').style.display='none';document.getElementById('btnNext').style.display='block';document.getElementById('btnNext').textContent='→ 下一招';updateCombatUI();saveState();return}
...
function v3ClashResult(idx){var c=state.combat,enemy=ENEMY_TEMPLATES[c.enemyId],mult=[1.3,1.1,1.0,0.9,0.7,2.0][idx],eMult=[0.7,0.9,1.0,1.1,1.3,2.0][idx];
  ...
  c.shield=Math.floor(c.ce*0.5);updateCombatUI();var rp=document.getElementById('resultPanel');rp.style.display='block';rp.innerHTML='<div class="rp-cat">⚔ 对拼结果</div><div class="rp-val">你:'+pDmg+'伤害 | 敌:'+eDmg+'伤害'+(shAbs>0?' (🛡护盾吸收:'+shAbs+')':'')+'</div><div class="rp-desc">击破:'+c.clockBK+'/6 败势:'+c.clockLB+'/6</div>';  c.log.push('[T'+c.round+' CLASH] 你:'+pDmg+' 敌:'+eDmg+' BK:'+c.clockBK+' LB:'+c.clockLB);if(c.burnout)c.log.push('[T'+c.round+'] ⚠ 熔断')
}
...
// ========================================================= V3 BUTTONS =========================================================
function updateBtnRow(){var b=document.getElementById('btnCombatRow'),c=state.combat;if(!b)return;if(!c||!c.active||c.phase!=='player_tech'){b.style.display='none';return}b.style.display='flex';document.getElementById('bcDomain').style.display=(c.burnout||c.domainUsed||!hasTrait('领域展开'))?'none':'inline-block';document.getElementById('bcMax').style.display=(c.burnout||!hasTrait('极之番'))?'none':'inline-block';document.getElementById('bcRCT').style.display=(c.burnout&&hasTrait('反转术式'))?'inline-block':'none';document.getElementById('bcEscape').style.display=(c.stance==='流转')?'inline-block':'none';document.getElementById('bcBindLoan').style.display=(c.bindLoanUsed||!hasTrait('束缚'))?'none':'inline-block';document.getElementById('bcBindStack').style.display=(!c.bindLoanUsed||!hasTrait('束缚'))?'none':'inline-block'}
