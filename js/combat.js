...
function updateCombatUI(){
  ...
  var er=document.getElementById('enemyRes');if(!er){er=document.createElement('div');er.id='enemyRes';er.style.cssText='font-size:9px;padding:2px 12px;display:flex;gap:8px';var cb=document.getElementById('combatBars');if(cb)cb.appendChild(er)}er.innerHTML='<span>敌体力:<b>'+c.enemyStamina+'</b></span><span>敌咒力:<b>'+c.enemyCe+'</b></span><span>敌胜率:<b>'+c.enemyWin+'</b></span><span>敌DZ:<b>'+Math.floor(c.enemyDangerZone)+'%</b></span>';
  var dzEl=document.getElementById('cbDZVal');if(dzEl){dzEl.textContent=Math.floor(c.dangerZone)+'%'+'+'+v3DangerGrowth().toFixed(1)+'%';var dzBar=document.getElementById('cbDZBar');if(dzBar)dzBar.style.width=c.dangerZone+'%'}
  var edzEl=document.getElementById('cbEDZVal');if(edzEl){edzEl.textContent=Math.floor(c.enemyDangerZone)+'%'+'+'+v3EnemyDangerGrowth().toFixed(1)+'%';var edzBar=document.getElementById('cbEDZBar');if(edzBar)edzBar.style.width=c.enemyDangerZone+'%'}
