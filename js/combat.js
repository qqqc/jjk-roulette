...
stop=function(){
  ...
  if(rt==='combat_repeatable'){
    if(rid==='p4_ptech'){var tech=item._tech;if(!tech){showToast('无效技法');return}
      ...
      if(c.stamina<=0){c.phase='enemy_stamina';document.getElementById('btnSpin').style.display='none';document.getElementById('btnNext').style.display='block';document.getElementById('btnNext').textContent='→ 敌体力';updateCombatUI();saveState();return}
